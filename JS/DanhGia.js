...
document.addEventListener("DOMContentLoaded", function () {
  ...
  courseReview.forEach((course) => {
    ...
    row.innerHTML = `
            <td>${course.stt}</td>
            <td class="courseID">${course.idCourse}</td>
            <td class="subjectName" style="cursor: pointer;" data-id=${course.idCourse}>${course.name}</td>
            <td>${course.tc}</td>
            <td>${course.name_teacher}</td>
            <td claas="reviewCount">${course.review}</td>
        `;
    // Thêm hàng vào tbody
  ...
  subjectNames.forEach(function (subject) {
    subject.addEventListener("click", function () {
      ...
      var courseId = subject.dataset.id;
      localStorage.setItem("selectedSubjectid", courseId);
